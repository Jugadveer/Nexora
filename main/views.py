...
def chatbot_api(request):
    ...
    if request.method == 'POST':
        ...
        if message:
            try:
                # Add debug logging
                logger.info(f"Received chatbot request: {message}")
                
                # Check if Groq client is available
                if not groq_client:
                    logger.error("Groq client not initialized")
                    return JsonResponse({'error': 'AI service unavailable', 'response': 'AI service is currently unavailable. Please try again later.'})
                
                response = ask_groqcloud(message)
                logger.info(f"Got response from Groq: {response[:50]}...")
                ...
                return JsonResponse({'error': 'Failed to get response', 'response': 'Failed to get response from AI. Please try again later.'}, status=500)
        else:
            return JsonResponse({'error': 'Empty message', 'response': 'Please enter a message.'}, status=400)
    
    return JsonResponse({'error': 'Method not allowed', 'response': 'Method not allowed'}, status=405)
